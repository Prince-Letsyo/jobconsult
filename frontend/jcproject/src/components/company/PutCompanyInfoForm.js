...
const PutCompanyInfoForm = () => {
  ...
  const { companyInfoId } = router.query;
  const { data: companyInfo } = useGetCompanyInfoByCompanyInfoIdQuery(
    parseInt(companyInfoId)
  );
  return (
    <Formik
      initialValues={{
        ...
        type_of_employer: companyInfo?.type_of_employer,
        hear_about: companyInfo?.hear_about,
      ...
      onSubmit={async (values, actions) => {
        try {
          await changeCompanyInfoInfo(values).unwrap();
        } catch (error) {}
      }}
    >
      {({ values }) => (
