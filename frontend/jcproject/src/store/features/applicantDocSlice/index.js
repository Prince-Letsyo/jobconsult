  ...
  endpoints: (builder) => ({
    getApplicationDocs: builder.query({
      ...
      providesTags: (result, error, arg) => [
        ...
        ...result.data.map((id) => ({ type: "ApplicationDoc", id })),
      ],
    }),
    ...
    changeApplicationDocInfo: builder.mutation({
      query: (initialUser) => ({
        url: `/applications/applicant-docs/${initialUser.id}/`,
        method: "PUT",
        body: {
          ...initialUser,
        },
...
export const selectApplicationDocsResult =
  applicationDocApiSlice.endpoints.getApplicationDocs.select();
